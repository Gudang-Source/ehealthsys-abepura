...
//            return $(html);
        ...
        var photoTemplate = '<div class="photo">' + '<div class="image-preview"><a class="fancybox" id="show7" href="" title=""><img src="" alt="show7"/></a></div><div class="caption">';
//        if (opts.hasName)photoTemplate += '<h5></h5>';
        photoTemplate += '<input type="hidden" id="photopemeriksaan_id">';        
        if (opts.hasDesc)photoTemplate += '<textarea name="deskripsi" readonly=readonly rows="4" cols="1" style="width:210px;height:45px;margin-top:2px;vertical-align:middle;" class="span3" id="photo_description" onclick="this.focus();this.select();" onblur="updatePhoto(this);"></textarea><br/><br/>';        
        photoTemplate += '</div><div class="actions">';
        if (opts.hasName || opts.hasDesc)photoTemplate += '<span class="btn btn-primary btn-mini"><i class="icon-pencil icon-white" onclick="setEdit(this);" style="cursor:pointer;" rel="tooltip" title="Klik untuk ubah gambar"></i></span> ';
        photoTemplate += '<span class="deletePhoto btn btn-danger btn-mini"><i class="icon-remove icon-white" style="cursor:pointer;" rel="tooltip" title="Klik untuk hapus gambar"></i></span>' +
        ...
        function addPhoto(id, src, name, description, rank) {
            ...
            $('a', photo).attr('href', src);
            $('a', photo).attr('title', description);
            if (opts.hasName) $('.caption h5', photo).text(name);
//            if (opts.hasDesc) $('.caption p', photo).text(description);
            if (opts.hasDesc) $('.caption #photo_description', photo).text(description);
            $('#photopemeriksaan_id', photo).val(id);
            $('#judulphoto').val(name);
...
//            if (l > 0)$editorModal.modal('show');
        ...
        function removePhotos(ids) {
            if(confirm('Apakah anda yakin akan menghapus gambar ?')){                
                $.ajax({
                    type: 'POST',
                    url: opts.deleteUrl,
        ...
        function updateGallery(ids) {
            if(confirm('Apakah anda yakin akan menampilkan gambar di gallery ?')){          
                $('.sorter',$gallery).addClass('animation-loading');
                $('#frame').addClass('animation-loading');
                $.ajax({
                type: 'POST',
                url: opts.updateGallery,
                data: 'id[]=' + ids.join('&id[]=') + csrfParams,
                success: function (t) {
                    if (t == 'OK') {
                        $('.sorter',$gallery).removeClass('animation-loading');
                        $('#frame').removeClass('animation-loading');
        ...
        function updateButtons() {
            ...
            if (selectedCount == 0) {
                $('.edit_selected, .remove_selected, .gallery_selected', $gallery).addClass('disabled');
            } else {
                $('.edit_selected, .remove_selected, .gallery_selected', $gallery).removeClass('disabled');
            }
        }
        ...
        $('.images', $sorter).sortable({ tolerance: "pointer" }).disableSelection().bind("sortstop", function () {
            ...
            $('.photo', $sorter).each(function () {
                var t = $(this);
                data.push('order[' + t.data('id') + ']=' + t.data('nourut'));
            });
            $.ajax({
                type: 'POST',
                url: opts.arrangeUrl,
                ...
                for (var i = 0; i < filesCount; i++) {
                    ...
                    xhr.onload = function () {
                        ...
                        if (this.status == 200) {
                            ...
                            addPhoto(resp['photopemeriksaan_id'], resp['preview'], resp['judulphotopemeriksaan'], resp['keteranganphoto'], resp['nourut']);
                            ids.push(resp['photopemeriksaan_id']);
                            document.getElementById('frame').contentWindow.location.reload(true);
                        } else if(resp['photopemeriksaan_id'] == ''){
        ...
        $('.gallery_selected', $gallery).click(function (e) {
                e.preventDefault();
                var ids = [];
                $('.photo.selected', $sorter).each(function () {
                    ids.push($(this).data('id'));
