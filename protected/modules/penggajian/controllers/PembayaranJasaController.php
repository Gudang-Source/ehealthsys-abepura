		...
		if(isset($_POST['GJPembayaranjasaT']) && isset($_POST['GJPembjasadetailT']))
		{
			...
			$model->rujukandariNama=$_POST['GJPembayaranjasaT']['rujukandariNama'];
                        ...
                        if($model->pilihDokter == "rujukan"){
                            $model->tgl_awalPenunjang = $_POST['GJPembayaranjasaT']['tgl_awalPenunjang'];
                            $model->tgl_akhirPenunjang = $_POST['GJPembayaranjasaT']['tgl_akhirPenunjang'];
                            $model->periodejasa = $format->formatDateTimeForDb($_POST['GJPembayaranjasaT']['tgl_awalPenunjang']);
                            $model->sampaidgn = $format->formatDateTimeForDb($_POST['GJPembayaranjasaT']['tgl_akhirPenunjang']);
                        }else if($model->pilihDokter == "rs"){
                            $model->tgl_awalPendaftaran = $_POST['GJPembayaranjasaT']['tgl_awalPendaftaran'];
                            $model->tgl_akhirPendaftaran = $_POST['GJPembayaranjasaT']['tgl_akhirPendaftaran'];
                            $model->periodejasa = $format->formatDateTimeForDb($_POST['GJPembayaranjasaT']['tgl_awalPendaftaran']);
                            $model->sampaidgn = $format->formatDateTimeForDb($_POST['GJPembayaranjasaT']['tgl_akhirPendaftaran']);
                        }
...
//                $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
        ...
        public function actionAddDetailPembayaranJasa()
        {
            if(Yii::app()->request->isAjaxRequest) { 
                ...
                if(!empty($rujukandariId)){
                    ...
                    $dataDetails = GJPasienpelayanandokterrsV::model()->findAll($criteria);
                    $criteria1 = $criteria;
					$criteria1->group .= ', daftartindakan_id, daftartindakan_kode, daftartindakan_nama, tarif_tindakan';
                    $criteria1->select = $criteria1->group;
					$criteria1->order .= ', daftartindakan_id';
                    $dataTindakans = GJPasienpelayanandokterrsV::model()->findAll($criteria1);
                    $criteria2 = $criteria;
                    $criteria2->select .= ", SUM(tarif_tindakankomp) AS tarif_tindakankomp";
                    $criteria2->addInCondition('komponentarif_id', $komponentarifIds);
                    $dataKomponen = GJPasienpelayanandokterrsV::model()->findAll($criteria2);
                    foreach($dataDetails AS $i => $dataDetail){
        ...
        public function actionLihatDetail($id){
            ...
            $judulLaporan = null;
            $this->render($this->path_view. 'Print',array('model'=>$model, 'modDetail'=>$modDetail,'judulLaporan'=>$judulLaporan,'frame'=>true));
        }
