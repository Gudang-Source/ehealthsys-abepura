...
<?php 
    ...
    $modNewRekenings[0] = new RekeningsupplierV;
?>
<script type="text/javascript">
    var namaModel = "RekeningsupplierV";
    var trRekening=new String(<?php echo CJSON::encode($this->renderPartial('akuntansi.views.fakturPembelianGU.rekening._rowRekening',array('modRekenings'=>$modNewRekenings,'removeButton'=>true),true));?>);
...
//        $("#"+namaModel+"_"+row+"_saldokredit").val(formatNumber(parseFloat(params.saldokredit)));
    ...
    function getDataRekeningFaktur(supplier_id, total){
...
//        $("#formJurnalRekeningKasir").addClass("srbacLoading");
        ...
        $.post('<?php echo Yii::app()->createUrl('ActionAjax/GetDataRekeningPembayaranSupplierUmum');?>', {supplier_id:supplier_id, total:total},
            function(data){
                if(data == null){
//                    alert("Tidak memiliki rekening !");
    ...
    function renameRowRekening(){
        ...
        $("#tblInputRekening > tbody").find('tr').each(
            function()
            {
                ...
                $(this).find('input[name$="[penjamin_id]"]').attr('name',namaModel+'['+i+'][penjamin_id]')
                        .attr('id',namaModel+'_'+i+'_penjamin_id');
                $(this).find('input[name$="[penjamin_nama]"]').attr('name',namaModel+'['+i+'][penjamin_nama]')
                        .attr('id',namaModel+'_'+i+'_penjamin_nama');
    ...
    function updateRekeningFaktur(supplier_id, saldo){
        $("#tblInputRekening > tbody > tr").each(function(){
            if($(this).find('input[name$="[supplier_id]"]').val() == supplier_id){
