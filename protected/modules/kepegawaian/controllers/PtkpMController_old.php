...
class PtkpMController extends MyAuthController
{
	...
	if(isset($_POST['KPPtkpM']))
		{
			$model->attributes=$_POST['KPPtkpM'];
	...
	{
		$dataProvider=new CActiveDataProvider('KPPtkpM');
	...
	public function actionAdmin($sukses='')
	{
		...
		if(isset($_GET['KPPtkpM']))
			$model->attributes=$_GET['KPPtkpM'];
	...
	{
		$model=KPPtkpM::model()->findByPk($id);
		if($model===null)
	...
	{
        //if(!Yii::app()->user->checkAccess(Params::DEFAULT_UPDATE)){throw new CHttpException(401,Yii::t('mds','You are prohibited to access this page. Contact Super Administrator'));}                                                        
        KPPtkpM::model()->updateByPk($id, array('berlaku'=>false));
        $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
	}
    ...
    {
        $model= new KPPtkpM;
        $model->attributes=$_REQUEST['KPPtkpM'];
