...
class KomponengajiMController extends MyAuthController
{
	/**
...
//                    $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
        ...
        if(isset($_POST['id']))
        {
           $update = KomponengajiM::model()->updateByPk($id,array('komponengaji_aktif'=>false));
           if($update)
            {
                if (Yii::app()->request->isAjaxRequest)
                {
        ...
        {
            $model= new KomponengajiM;
            $model->attributes=$_REQUEST['KomponengajiM'];
            $judulLaporan='Data KomponengajiM';
