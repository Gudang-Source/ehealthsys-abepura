...
class SusunankelMController extends MyAuthController
{
	/**
...
//                    echo print_r($modPegawai->attributes);
                    if (count($modPegawai) == 1){
                        $model->pegawai_id = $id;
                        $ordernourut = array('order'=>'nourutkel ASC');
                        $details = KPSusunankelM::model()->findAllByAttributes(array('pegawai_id'=>$id),$ordernourut);
                    }
                }
		...
		{
			$model->attributes=$_POST['KPSusunankelM'];
                        $model->pegawai_id = $_POST['KPSusunankelM']['pegawai_id'];
                        $modPegawai->attributes = $_POST['KPPegawaiM'];
//                        echo "<pre>";
//                        echo print_r($_POST['KPSusunankelM']);
//                        exit();
                        $details = $this->validasiTabular($_POST['KPSusunankelM'], $modPegawai);
...
//            exit();
        ...
        public function actiondeleteKeluarga($id)
                {
                    $model = new KPSusunankelM;
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['KPSusunankelM']))
		{
			...
			if($model->save()){
                                Yii::app()->user->setFlash('success', '<strong>Berhasil!</strong> Data berhasil disimpan.');
				$this->redirect(array('view','id'=>$model->susunankel_id));
                        }
		}
	...
	{
		$dataProvider=new CActiveDataProvider('KPSusunankelM');
	...
	public function actionAdmin()
	{
		...
		$model=new KPSusunankelM('search');
	...
	{
		$model=KPSusunankelM::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kpsusunankel-m-form')
		{
        ...
        public function actionRemoveTemporary($id)
	{
        ...
        {
            $model= new KPSusunankelM;
            $model->attributes=$_REQUEST['KPSusunankelM'];
            $judulLaporan='Data KPSusunankelM';
