...
class RuanganMController extends MyAuthController
{
	...
	public function actionRemoveTemporary($id)
	{
            ...
            KPRuanganM::model()->updateByPk($id, array('ruangan_aktif'=>false));
	...
	{
                //if(!Yii::app()->user->checkAccess(Params::DEFAULT_CREATE)){throw new CHttpException(401,Yii::t('mds','You are prohibited to access this page. Contact Super Administrator'));}                                  
		$model=new KPRuanganM;
                $modRiwayatRuangan=new KPRiwayatRuanganR;

		// Uncomment the following line if AJAX validation is needed
		
                if(isset($_POST['KPRuanganM']))
                {
                  $transaction = Yii::app()->db->beginTransaction();
                  try {
                        $modRiwayatRuangan=new KPRiwayatRuanganR; 
                        $modRiwayatRuangan->attributes=$_POST['KPRiwayatRuanganR'];
                        $modRiwayatRuangan->save();
                        $valid=true;
                        foreach($_POST['KPRuanganM'] as $i=>$item):
                            if(is_integer($i)) {
                              $model=new KPRuanganM;
                              $random=rand(0000000,9999999);
                              $model->attributes=$_POST['KPRuanganM'][$i];
                              $model->instalasi_id=$_POST['KPRuanganM']['instalasi_id'];
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['KPRuanganM']))
		{
                        ...
                        $transaction = Yii::app()->db->beginTransaction();
                            try {
                                    $model->attributes=$_POST['KPRuanganM'];
                                    $model->save();
                                    $jumlahKelasPelayanan=COUNT($_POST['kelaspelayanan_id']);
                                    $jumlahJenisKasusPenyakit=COUNT($_POST['jeniskasuspenyakit_id']);
	...
	{
		$dataProvider=new CActiveDataProvider('KPRuanganM');
	...
	public function actionAdmin()
	{
		...
		$model=new KPRuanganM('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['KPRuanganM']))
			$model->attributes=$_GET['KPRuanganM'];
	...
	{
		$model=KPRuanganM::model()->findByPk($id);
		if($model===null)
