<div class="white-container">
    <legend class="rim2">Lihat <b>Status Kehadiran</b></legend>
    <?php
    $this->breadcrumbs=array(
            'Statuskehadiran Ms'=>array('index'),
            $model->statuskehadiran_id,
    );

    $arrMenu = array();
    //                array_push($arrMenu,array('label'=>Yii::t('mds','View').' Status Kehadiran', 'header'=>true, 'itemOptions'=>array('class'=>'heading-master'))) ;
    //                array_push($arrMenu,array('label'=>Yii::t('mds','List').' StatuskehadiranM', 'icon'=>'list', 'url'=>array('index'))) ;
    //                (Yii::app()->user->checkAccess(Params::DEFAULT_CREATE)) ?array_push($arrMenu,array('label'=>Yii::t('mds','Create').' StatuskehadiranM', 'icon'=>'file', 'url'=>array('create'))) :  '' ;
    //                (Yii::app()->user->checkAccess(Params::DEFAULT_UPDATE)) ?array_push($arrMenu,array('label'=>Yii::t('mds','Update').' StatuskehadiranM', 'icon'=>'pencil','url'=>array('update','id'=>$model->statuskehadiran_id))) :  '' ;
    //                array_push($arrMenu,array('label'=>Yii::t('mds','Delete').' StatuskehadiranM','icon'=>'trash','url'=>'#','linkOptions'=>array('submit'=>array('delete','id'=>$model->statuskehadiran_id),'confirm'=>Yii::t('mds','Are you sure you want to delete this item?')))) ;
                    // (Yii::app()->user->checkAccess(Params::DEFAULT_ADMIN)) ?array_push($arrMenu,array('label'=>Yii::t('mds','Manage').' Status Kehadiran', 'icon'=>'folder-open', 'url'=>array('admin'))) :  '' ;

    $this->menu=$arrMenu;

    $this->widget('bootstrap.widgets.BootAlert'); ?>

    <?php $this->widget('ext.bootstrap.widgets.BootDetailView',array(
            'data'=>$model,
            'attributes'=>array(
                    'statuskehadiran_id',
                    'statuskehadiran_nama',
                    'statuskehadiran_singkatan',
    //		'statuskehadiran_aktif',
                                    array(
                                        'label'=>'Aktif',
                                        'value'=>(($model->statuskehadiran_aktif==1)? "Ya" : "Tidak"),
                                    ),
            ),
    )); ?>

    <?php 
    echo CHtml::link(Yii::t('mds', '{icon} Pengaturan Status Kehadiran', array('{icon}'=>'<i class="icon-file icon-white"></i>')), $this->createUrl(Yii::app()->controller->id.'/admin',array('modul_id'=> Yii::app()->session['modul_id'])), array('class'=>'btn btn-success'))."&nbsp";
    $this->widget('UserTips',array('type'=>'view'));?>
</div>