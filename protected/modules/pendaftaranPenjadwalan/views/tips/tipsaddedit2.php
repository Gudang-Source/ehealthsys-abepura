<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table border="0" style="padding :none;">
  <tr>
    <td width="10px">1. </td>
    <td>Gunakan tombol ini  <button class="btn btn-primary" name="yt0" type="submit"><i class="icon-search icon-white"></i>
      </button> untuk melakukan pencarian.</td>
  </tr>
  <tr>
    <td>2. </td>
    <td>Gunakan tombol ini  <button class="btn btn-danger" name="yt0" type="submit"><i class="icon-plus-sign icon-white"></i>
      </button> untuk menambah baris.</td>
  </tr>
  <tr>
    <td>3. </td>
    <td>Gunakan tombol ini  <button class="btn btn-primary" name="yt0" type="submit"><i class="icon-minus-sign icon-white"></i>
      </button> untuk menghapus baris.</td>
  </tr>
  <tr>
    <td>4. </td>
    <td>Gunakan tombol ini  <div class="btn btn-primary"><i class="icon-ok icon-white"></i>
Simpan
</div> berfungsi untuk menyimpan.</td>
  </tr>
  <tr>
    <td>5. </td>
    <td>Gunakan tombol ini <div class="btn btn-danger">
      <!--<i class="icon-ban-circle icon-white"></i>
      Batal-->
      <i class="icon-refresh icon-white"></i>
      Ulang      
      </div> berfungsi untuk kembali.</td>
  </tr>
</table>
</td>
  </tr>
</table>
</p>