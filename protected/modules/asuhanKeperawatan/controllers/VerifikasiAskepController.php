...
//		echo "</pre>";
//		exit;
		if (isset($_POST['ASVerifikasiaskepT']) && !empty($_POST['ASPendaftaranT']['pendaftaran_id'])) {
			...
			try {
				...
				$model->pendaftaran_id = $_POST['ASPendaftaranT']['pendaftaran_id'];
				$model->pengkajianaskep_id = $_POST['ASPengkajianaskepT']['pengkajianaskep_id'];
				$model->rencanaaskep_id = $_POST['ASRencanaaskepT']['rencanaaskep_id'];
				$model->implementasiaskep_t = $_POST['ASImplementasiaskepT']['implementasiaskep_id'];
				$model->evaluasiaskep_t = $_POST['ASEvaluasiaskepT']['evaluasiaskep_id'];
						...
						if (isset($_POST['ASRencanaaskepdetT'])) {
							...
							foreach ($_POST['ASRencanaaskepdetT'] as $i => $rencanadet) {
								if (!empty($rencanadet['rencanaaskepdet_id'])) {
									...
									$modRencanaDet = new ASRencanaaskepdetT;
									$modRencanaDet->attributes = $rencanadet;
									$modRencanaDet->rencanaaskep_id = $_POST['ASRencanaaskepT']['rencanaaskep_id'];
								}
//								$modRencanaDet = $modRencanaDet->save();
								if ($modRencanaDet->save()) {
									...
									if (isset($_POST['ASImplementasiaskepT'])) {
										...
										$implementasi = $this->saveImplementasi($_POST['ASImplementasiaskepT']);
									...
									if (isset($_POST['ASEvaluasiaskepT'])) {
										$evaluasi = $this->saveEvaluasi($_POST['ASEvaluasiaskepT']);
...
//                    $penjamin = PenjaminpasienM::model()->findAll();
	...
	public function actionGetDiagnosaImplementasi($diagnosakep_id = null) {
		if (Yii::app()->request->isAjaxRequest) {
			...
			$data['form'] .= "<br>";
			$bk_head = AlternatifdxM::model()->findAllByAttributes(array('alternatifdx_aktif' => true, 'diagnosakep_id' => $diagnosakep_id));
			if (count($bk_head)) {
				foreach ($bk_head as $i => $bk) {
					$data['form'] .= "<ul class='spasi1'>";
					$data['form'] .= '<li >' . $bk->alternatifdx_nama . '</li>';
					$data['form'] .= "</ul>";
				}
			} else {
				$data['form'] .= "<ul class='spasi1'>";
...
//									WHERE rencanaaskepdet_id =' . $modImplementasiDet->rencanaaskepdet_id . ' AND pilihrencanaaskep_t.intervensidet_id IS NOT NULL');
	...
	public function actionLoadEvaluasi($implementasiaskep_id) {
	...
	public function saveImplementasi($post) {
		$modImplementasi = ASImplementasiaskepT::model()->findByPk($post['implementasiaskep_id']);
		$modImplementasi->attributes = $_POST['ASImplementasiaskepT'];
		$modImplementasi->pegawai_id = $_POST['ASImplementasiaskepT']['pegawai_id'];
		$modImplementasi->implementasiaskep_tgl = MyFormatter::formatDateTimeForDb($_POST['ASImplementasiaskepT']['implementasiaskep_tgl']);
	...
	public function saveEvaluasi($post) {
