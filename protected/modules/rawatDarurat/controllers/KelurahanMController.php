...
class KelurahanMController extends MyAuthController
{
	...
	public function actionCreate()
	{
		...
		$model=new RDKelurahanM;

		// Uncomment the following line if AJAX validation is needed
		

		if(isset($_POST['RDKelurahanM']))
		{
                                      $valid=true;
                                        foreach($_POST['RDKelurahanM'] as $i=>$item)
                                        {
                                            if(is_integer($i)) 
                                            {
                                                $model=new RDKelurahanM;
                                                if(isset($_POST['RDKelurahanM'][$i]))
                                                    $model->attributes=$_POST['RDKelurahanM'][$i];
                                                	$model->kelurahan_aktif = true;
                                                    $model->kecamatan_id = $_POST['RDKelurahanM']['kecamatan_id'];
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['RDKelurahanM']))
		{
			$model->attributes=$_POST['RDKelurahanM'];
                                                $model->kelurahan_aktif = $_POST['RDKelurahanM']['kelurahan_aktif'];
			if($model->save()){
                                Yii::app()->user->setFlash('success', '<strong>Berhasil!</strong> Data berhasil disimpan.');
	...
	{
		$dataProvider=new CActiveDataProvider('RDKelurahanM');
	...
	public function actionAdmin()
	{
		...
		$model=new RDKelurahanM('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['RDKelurahanM']))
			$model->attributes=$_GET['RDKelurahanM'];
	...
	{
		$model=RDKelurahanM::model()->findByPk($id);
		if($model===null)
	...
	protected function performAjaxValidation($model)
	{
		...
		//                    $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
		        ...
		        if(isset($_POST['id']))
		        {
		           $update = RDKelurahanM::model()->updateByPk($id,array('kelurahan_aktif'=>false));
		           if($update)
		            {
		                if (Yii::app()->request->isAjaxRequest)
		                {
            ...
            public function actionPrint()
                {
                 ...
                 $model= new RDKelurahanM;
                 $model->attributes=$_REQUEST['RDKelurahanM'];
