                ...
                <?php 
                    $this->widget('MyJuiAutoComplete', array(
                                    ...
                                    'source'=>'js: function(request, response) {
                                       $.ajax({
                                           url: "'.Yii::app()->createUrl('rawatDarurat/PembebasanTarifRD/daftarPasienTindakanRuangan').'",
                                           dataType: "json",
                                           data: {
                                               term: request.term,
                                           },
...
if(isset($_GET['RDInfokunjunganrdV'])){
    ...
    if(isset($_GET['RDInfokunjunganrdV']['tgl_pendaftaran']))
            ...
            'no_rekam_medik',   
                ...
                array(
                 ...
                 array(
                   ...
                   'filter'   => CHtml::dropDownList('RDInfokunjunganrdV[penjamin_id]',$modDataPasien->penjamin_id,CHtml::listData(PenjaminpasienM::model()->findAll("penjamin_aktif = TRUE ORDER BY penjamin_nama ASC"), 'penjamin_id', 'penjamin_nama'),array('empty'=>'--Pilih--'))
                 ),
                 array(
                     'name'=>'nama_pegawai',
...
function isiDataPasien(data)
{
    ...
    $.post('<?php echo Yii::app()->createUrl('rawatDarurat/PembebasanTarifRI/loadTindakanKomponenPasien');?>', {pendaftaran_id:data.pendaftaran_id}, function(data){
...
{
    $.post("<?php echo Yii::app()->createUrl('rawatDarurat/PembebasanTarifRD/loadDataPasien');?>", {no_rekam_medik:params },
        function(data){
            $('#RJPendaftaranT_tgl_pendaftaran').val(data.tgl_pendaftaran);
            $('#RJPendaftaranT_no_pendaftaran').val(data.no_pendaftaran);
            $('#RJPendaftaranT_umur').val(data.umur);
...
function setDataPasien(){
	...
	if(no_rekam_medik != '' || pendaftaran_id != ''){
		$.post('<?php echo Yii::app()->createUrl('rawatDarurat/PembebasanTarifRD/loadTindakanKomponenPasien');?>', {pendaftaran_id:pendaftaran_id, pegawai_id:pegawai_id}, function(data){
			$('#divTarifPasien').html(data.tabelPembebasanTarif);        
			$("#tblTindakanPasien .integer").maskMoney({"symbol":"","defaultZero":true,"allowZero":true,"decimal":".","thousands":",","precision":0});
			$("#tblTindakanPasien .integer").each(function(){this.value = formatNumber(this.value)});
		}, 'json');
