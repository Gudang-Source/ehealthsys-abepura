<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table border="0" style="padding :none;">
  <tr>
    <td>1. </td>
    <td>Icon <i class="icon-calendar"></i>untuk menentukan tanggal transaksi.</td>
  </tr>
  <tr>
    <td>2. </td>
    <td>Icon  <i class="icon-form-ubah"></i> berfungsi untuk mengubah data.</td>
  </tr>
  <tr>
    <td>3. </td>
    <td>Icon  <i class="icon-form-rincianjual"></i> berfungsi untuk melihat rincian detail penjualan.</td>
  </tr>
<tr>
    <td>4. </td>
    <td>Icon  <i class="icon-form-silang"></i> berfungsi untuk melakukan pembatalan.</td>
  </tr>
  <tr>
    <td>5. </td>
    <td>Icon  <i class="icon-form-retur"></i> berfungsi untuk melanjutkan ke retur penjualan.</td>
  </tr>
  <tr>
    <td>6. </td>
    <td>Gunakan tombol ini  <button class="btn btn-primary" name="yt0" type="submit"><i class="icon-search icon-white"></i>
Cari
</button> berfungsi untuk mencari data</td>
  </tr>
  <tr>
    <td>7. </td>
    <td>Gunakan tombol ini <a class="btn btn-danger" href="/simrs/index.php?r=rawatInap/pasienRawatInap/formTindakLanjutDariPasienRI">
<i class="icon-refresh icon-white"></i>
Ulang
</a> untuk mengulang kembali inputan.</td>
  </tr>
</table>
</p>