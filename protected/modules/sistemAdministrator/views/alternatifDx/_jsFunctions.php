<script type="text/javascript">
	...
	function hapusLookup(obj){
		var alternatifdx_id = $(obj).parents("tr").find("input[name$='[alternatifdx_id]']").val();
		if(alternatifdx_id !== ""){
			...
			function(r){
				if(r){
					$.ajax({
						type:'POST',
						url:'<?php echo $this->createUrl('Delete'); ?>&id='+alternatifdx_id,
						data: {id : alternatifdx_id},//
						dataType: "json",
						success:function(data){
							if(data.sukses == 1){
	...
	function cek(obj) {
		if ($(obj).is(':checked')) {
			$(obj).parents("tr").find("input[name$='[alternatifdx_aktif]']").val(1);
		} else {
			$(obj).parents("tr").find("input[name$='[alternatifdx_aktif]']").val(0);
		}
	}
	...
	$(document).ready(function(){
		...
		<?php if(!empty($model->alternatifdx_id)){ ?>
				refreshTable();
		<?php } ?>
	})
