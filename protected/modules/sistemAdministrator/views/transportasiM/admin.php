...
$arrMenu = array();
				(Yii::app()->user->checkAccess(Params::DEFAULT_ADMIN)) ?array_push($arrMenu,array('label'=>Yii::t('mds','Manage').' Transportasi ', 'header'=>true, 'itemOptions'=>array('class'=>'heading-master'))) :  '' ;
...
$('.search-button').click(function(){
		$('.search-form').toggle();
	$('#SATransportasiM_lookup_name').focus();
		return false;
});
$('.search-form form').submit(function(){
		$.fn.yiiGridView.update('rdtransportasi-m-grid', {
				data: $(this).serialize()
		});
		return false;
});
...
<!--<div class="block-tabel">-->
	<!--<h6>Tabel <b>Transportasi</b></h6>-->
	<?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
		'id'=>'rdtransportasi-m-grid',
		'dataProvider'=>$model->searchTransportasi(),
		'enableSorting'=>true,
		'filter'=>$model,
		'template'=>"{summary}\n{items}\n{pager}",
		...
		'columns'=>array(
				...
				array(
						...
						'buttons'=>array(
							'view' => array (
										  'options'=>array('title'=>'Klik untuk melihat transportasi'),
										),
						 ),
				),
				array(
						...
						'buttons'=>array(
							'update' => array (
										  'visible'=>'Yii::app()->controller->checkAccess(array("action"=>Params::DEFAULT_UPDATE))',
											'options'=>array('title'=>'Klik untuk mengubah transportasi'),
										),
						 ),
				),
				array(
...
<!--</div>-->
    <?php 
    echo CHtml::link(Yii::t('mds','{icon} Tambah Transportasi',array('{icon}'=>'<i class="icon-plus icon-white"></i>')), 
...
{
    $("#rdtransportasi-m-search :input[name='"+ obj.name +"']").val(obj.value);
}
function print(caraPrint)
{
    window.open("${urlPrint}/"+$('#rdtransportasi-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
