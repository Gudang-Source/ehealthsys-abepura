<script type="text/javascript">
	...
	function hapusLookup(obj) {
		var faktorrisikodet_id = $(obj).parents("tr").find("input[name$='[indikatorimplkepdet_id]']").val();
			...
			myConfirm("Apakah anda yakin akan menghapus data ini?", "Perhatian!",
					function (r) {
						if (r) {
							...
							$.ajax({
								type: 'POST',
								url: '<?php echo $this->createUrl('Delete'); ?>&id=' + faktorrisikodet_id,
								data: {id: faktorrisikodet_id}, //
								dataType: "json",
								success: function (data) {
									if (data.sukses == 1) {
										$(obj).parents('tr').detach();
	...
	function cek(obj) {
		if ($(obj).is(':checked')) {
			$(obj).parents("tr").find("input[name$='[indikatorimplkepdet_aktif]']").val(1);
		} else {
			$(obj).parents("tr").find("input[name$='[indikatorimplkepdet_aktif]']").val(0);
		}
	}
	...
	$(document).ready(function () {
		tambahLookup();
<?php if (!empty($model->implementasikep_id)) { ?>
			//setLookup('<?php //echo $model->implementasikep_id; ?>');
<?php } ?>
