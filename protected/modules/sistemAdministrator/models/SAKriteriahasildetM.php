...
class SAKriteriahasildetM extends KriteriahasildetM {
	...
	public function criteriaSearch()
	{
		...
		$criteria->compare('kriteriahasil.diagnosakep_id',$this->diagnosakep_id,true);
		...
		if (!empty($this->aktif)) {
			if ($this->aktif == 1) {
				$criteria->addCondition('t.kriteriahasildet_aktif = TRUE');
			}
			if ($this->aktif == 0) {
				$criteria->addCondition('t.kriteriahasildet_aktif = FALSE');
			} 
		}
		return $criteria;
	}
