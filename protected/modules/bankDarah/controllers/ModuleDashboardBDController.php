...
class ModuleDashboardBDController extends ModuleDashboardNeonController
{
    ...
    public function actionSetIFrameDashboard(){
        ...
        $sql = "SELECT COUNT(pasien_id) AS jumlah
                FROM pasienmasukpenunjang_v
                WHERE DATE(tgl_pendaftaran) = '".date('Y-m-d')."' and instalasiasal_id = ".Params::INSTALASI_ID_LAB;
        $result = Yii::app()->db->createCommand($sql)->queryRow();
...
//				ORDER BY carabayar_nama ASC";
		...
		$sql = "SELECT COUNT(pendaftaran_id) AS jumlah
				FROM pasienmasukpenunjang_v
				WHERE DATE(tgl_pendaftaran) BETWEEN '".date("Y-m")."-01' AND '".date("Y-m-d")."'
					AND instalasi_id = ".Params::INSTALASI_ID_LAB." AND instalasiasal_id <> ".Params::INSTALASI_ID_LAB;
		$result = Yii::app()->db->createCommand($sql)->queryRow();
		...
		$dataTable = new BDPasienMasukPenunjangV("searchPemeriksaan");
