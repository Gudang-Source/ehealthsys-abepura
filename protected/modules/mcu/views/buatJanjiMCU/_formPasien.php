...
<fieldset class="box" id='fieldsetPasien'>
    <legend class="rim">Masukkan Data Pasien</legend>
    <div class="row-fluid">
            <div class="span4">        
                <div class="control-group ">
					...
					<?php echo $form->dropDownList($modPasien,'propinsi_id', CHtml::listData($modPasien->getPropinsiItems(), 'propinsi_id', 'propinsi_nama'), 
													  array('class'=>'span4','empty'=>'-- Pilih --', 'onkeyup'=>"return $(this).focusNextInputField(event)", 
															'ajax'=>array('type'=>'POST',
																		  'url'=>$this->createUrl('GetKabupaten',array('encode'=>false,'namaModel'=>'MCPasienM')),
																		  'update'=>'#MCPasienM_kabupaten_id',),
				...
				<div class="control-group ">
					...
					<div class="controls">
						...
						<?php echo $form->dropDownList($modPasien,'kabupaten_id',CHtml::listData($modPasien->getKabupatenItems($modPasien->propinsi_id), 'kabupaten_id', 'kabupaten_nama'),
														  array('class'=>'span4','empty'=>'-- Pilih --', 'onkeyup'=>"return $(this).focusNextInputField(event)",
																'ajax'=>array('type'=>'POST',
																			  'url'=>$this->createUrl('GetKecamatan',array('encode'=>false,'namaModel'=>'MCPasienM')),
																			  'update'=>'#MCPasienM_kecamatan_id'),
				...
				<div class="control-group ">
					...
					<div class="controls">
						...
						<?php echo $form->dropDownList($modPasien,'kecamatan_id',CHtml::listData($modPasien->getKecamatanItems($modPasien->kabupaten_id), 'kecamatan_id', 'kecamatan_nama'),
														  array('class'=>'span4','empty'=>'-- Pilih --', 'onkeyup'=>"return $(this).focusNextInputField(event)",
																'ajax'=>array('type'=>'POST',
																			  'url'=>$this->createUrl('GetKelurahan',array('encode'=>false,'namaModel'=>'MCPasienM')),
																			  'update'=>'#MCPasienM_kelurahan_id'))); ?>
...
{
    var statusperkawinan = $('#MCPasienM_statusperkawinan').val();
    var namadepan = $('#MCPasienM_namadepan');
    var umur = $("#<?php echo CHtml::activeId($modPasien,'umur');?>").val().substr(0,2);
    umur = parseInt(umur);
    if(umur <= 5){
        var namadepan = $('#MCPasienM_namadepan').val('BY. Ny.');
        if(statusperkawinan.length > 0){
            $('#MCPasienM_statusperkawinan').val('');
            myAlert('Maaf status perkawinan belum cukup usia');
        }
    }else if(umur <= 15){
        var namadepan = $('#MCPasienM_namadepan').val('An.');
        if(statusperkawinan.length > 0){
            $('#MCPasienM_statusperkawinan').val('');
            myAlert('Maaf status perkawinan belum cukup usia');
        }
    }else{
        if($('#MCPasienM_jeniskelamin_0').is(':checked')){
            if(statusperkawinan !== 'JANDA'){
                var namadepan = $('#MCPasienM_namadepan').val('Tn.');
            }else{
                myAlert('Pilih status pernikahan yang sesuai');
                $('#MCPasienM_statusperkawinan').val('KAWIN');
                var namadepan = $('#MCPasienM_namadepan').val('Tn.');
            }
        }
        ...
        if($('#MCPasienM_jeniskelamin_1').is(':checked')){
            if(statusperkawinan !== 'DUDA'){
                if(statusperkawinan === 'KAWIN' || statusperkawinan == 'JANDA' || statusperkawinan == 'NIKAH SIRIH' || statusperkawinan == 'POLIGAMI'){
                    var namadepan = $('#MCPasienM_namadepan').val('Ny.');
                }else{
                    var namadepan = $('#MCPasienM_namadepan').val('Nn');
                }                
            }else{
                myAlert('Pilih status pernikahan yang sesuai');
                $('#MCPasienM_statusperkawinan').val('KAWIN');
...
{
    var is_true = true;
    var namadepan = $('#MCPasienM_namadepan').val();
    if(namadepan.length != 0)
    {
        if(obj.value == 'PEREMPUAN')
        {
            if(namadepan != 'Nn.' && namadepan != 'Ny.' && namadepan != 'BY. Ny.')
            {
                ...
                $('#MCPasienM_jeniskelamin_0').attr('checked',true);
                is_true = false;
            }
        }else{
            ...
            {
                myAlert('Pilih Jenis kelamin yang sesuai');
                $('#MCPasienM_jeniskelamin_1').attr('checked',true);
                is_true = false;
            }
        }
    }else{
...
function setValueStatus(obj)
{
    ...
    if(gelar === 'Tn.')
    {
        ...
        $('#MCPasienM_jeniskelamin_0').attr('checked',true);
        $('#MCPasienM_statusperkawinan').val('BELUM KAWIN');
    }else{
...
{
    $('#MCPasienM_kecamatan_id').find('option').remove().end().append('<option value="">-- Pilih --</option>').val('');
}

function clearKelurahan()
{
    $('#MCPasienM_kelurahan_id').find('option').remove().end().append('<option value="">-- Pilih --</option>').val('');
}
...
{   
    ...
    $.post("${urlGetTglLahir}",{umur: obj.value},
        function(data){
           $('#MCPasienM_tanggal_lahir').val(data.tglLahir); 
    },"json");
}
