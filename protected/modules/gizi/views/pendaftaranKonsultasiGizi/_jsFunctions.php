...
/** control accordion karcis konsultasi gizi*/
...
function setKarcis()
{
    var pasien_id=$("#<?php echo CHtml::activeId($modPasien,"pasien_id");?>").val();
    var penjamin_id=$("#<?php echo CHtml::activeId($model,"penjamin_id");?>").val();
    var ruangan_id = $("#form-penunjang").find('input[name$="[ruangan_id]"]').val();
    var kelaspelayanan_id = $("#form-penunjang").find('select[name$="[kelaspelayanan_id]"]').val();
...
function refreshDaftarPasien(){
    $.fn.yiiGridView.update('pendaftarterakhir-grid', {
        data: $(this).serialize()
    });
    return false;
}
...
/**
 * Set checklist tindakan lab
 * obj = div yang berisi elemen ruangan_id, kelaspelayanan_id
 */
function SetChecklistTindakan(obj){
    var penjamin_id = $("#<?php echo CHtml::activeId($model, 'penjamin_id') ?>").val();
    ...
    if(penjamin_id == ""){
        ...
        $("#dialog-pilihtindakan").parent().addClass("animation-loading");
        $('#dialog-pilihtindakan').dialog('open'); 
        $.fn.yiiGridView.update('tariftindakanperdaruangan', {
            data: {
                "GZTarifTindakanPerdaRuanganV[ruangan_id]":ruangan_id,
            ...
            complete: function(jqXHR, status) {
                if (status=='success'){
                    $("#dialog-pilihtindakan").parent().removeClass("animation-loading");
...
/**
 * Centang tindakan konsultasi gizi dari checkboxlist
 */
function pilihTindakanIni(kelompoktindakan_id,daftartindakan_id,kelompoktindakan_nama,daftartindakan_kode,daftartindakan_nama,harga_tariftindakan,jenistarif_id){
    var kelompoktindakan_id = kelompoktindakan_id;
    var kelompoktindakan_nama = kelompoktindakan_nama;
    var daftartindakan_id = daftartindakan_id;
    var daftartindakan_nama = daftartindakan_nama;
    var jenistarif_id = jenistarif_id;
    var harga_tariftindakan = harga_tariftindakan;
    var rowtindakan = '<?php echo CJSON::encode($this->renderPartial($this->path_view.'_rowTindakanPelayanan',array('modTindakan'=>$modTindakan),true));?>';
    //check tindakan 
    var cek_row = $("#form-tindakanpelayanan").find('input[name$="[daftartindakan_id]"][value="'+daftartindakan_id+'"]').val();
    if(cek_row){
        myAlert("Tindakan ini sudah dipilih sebelumnya!");
    }else{
        $("#form-tindakanpelayanan").find('tbody').append(rowtindakan);
        $("#form-tindakanpelayanan").find('input[name$="[ii][tindakanpelayanan_id]"]').val("");
        $("#form-tindakanpelayanan").find('input[name$="[ii][kelompoktindakan_id]"]').val(kelompoktindakan_id);
        $("#form-tindakanpelayanan").find('input[name$="[ii][daftartindakan_id]"]').val(daftartindakan_id);
        $("#form-tindakanpelayanan").find('input[name$="[ii][jenistarif_id]"]').val(jenistarif_id);
        $("#form-tindakanpelayanan").find('input[name$="[ii][satuantindakan]"]').val("<?php echo Params::SATUAN_TINDAKAN_LABORATORIUM; ?>");
        $("#form-tindakanpelayanan").find('input[name$="[ii][tarif_satuan]"]').val(harga_tariftindakan);
        $("#form-tindakanpelayanan").find('input[name$="[ii][tarif_tindakan]"]').val(formatInteger(harga_tariftindakan));
        $("#form-tindakanpelayanan").find('span[name$="[ii][daftartindakan_nama]"]').html(daftartindakan_nama);
        renameInputRow($("#form-tindakanpelayanan"));
    }

}

function batalTindakan(obj){
 var daftartindakan_nama = $(obj).parents("tr").find("span[name$='daftartindakan_nama']").text();
    myConfirm('Apakah anda akan menghapus tindakan konsultasi ' + daftartindakan_nama + ' ?','Perhatian!',
    function(r){
        if(r){
...
{
    window.open('<?php echo $this->createUrl('printStatus'); ?>&pendaftaran_id='+pendaftaran_id,'printwin','left=100,top=100,width=480,height=640');
}
...
function autoPrint(){
    ...
    <?php if(Yii::app()->user->getState('printkunjunganlsng')==TRUE){ ?>
        window.open('<?php echo $this->createUrl('printStatus',array('pendaftaran_id'=>$model->pendaftaran_id)); ?>','printwin','left=100,top=100,width=480,height=640');
    <?php  } ?>
}
