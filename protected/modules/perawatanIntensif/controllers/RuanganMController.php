...
class RuanganMController extends MyAuthController
{
        ...
        public function actionUpdate($id)
	{
           ...
           $model= RIRuanganM::model()->findByPk($id); 
	...
	{
            //if(!Yii::app()->user->checkAccess(Params::DEFAULT_UPDATE)){throw new CHttpException(401,Yii::t('mds','You are prohibited to access this page. Contact Super Administrator'));}                                             
            RIRuanganM::model()->updateByPk($id, array('ruangan_aktif'=>false));
		...
		// Uncomment the following line if AJAX validation is needed
                ...
                if(isset($_POST['RIRuanganM']))
                {
                  ...
                  try {
                        ...
                        $valid=true;
                        foreach($_POST['RIRuanganM'] as $i=>$item):
                            if(is_integer($i)) {
                              $model=new RIRuanganM;
                              $random=rand(0000000,9999999);
                              $model->attributes=$_POST['RIRuanganM'][$i];
                              $model->instalasi_id=$_POST['RIRuanganM']['instalasi_id'];
...
//                        $transaction = Yii::app()->db->beginTransaction();
//                            try {
//                                    $model->attributes=$_POST['RIRuanganM'];
//                                    $model->save();
//                                    $jumlahKelasPelayanan=COUNT($_POST['kelaspelayanan_id']);
//                                    $jumlahJenisKasusPenyakit=COUNT($_POST['jeniskasuspenyakit_id']);
...
//                            $this->loadModel($id)->delete(); 
	...
	{
		$dataProvider=new CActiveDataProvider('RIRuanganM');
...
//		$model->unsetAttributes();  // clear any default values
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionPrint()
        {
            ...
            $model= new RIRuanganM();
            $model->ruangan_id = Yii::app()->user->getState('ruangan_id');
            $model->attributes=$_GET['RIRuanganM'];
