...
function pilihTindakanIni(kelompoktindakan_id,daftartindakan_id,kelompoktindakan_nama,daftartindakan_kode,daftartindakan_nama,harga_tariftindakan,jenistarif_id){
    ...
    var rowtindakan = '<?php echo CJSON::encode($this->renderPartial('_rowTindakanPelayanan',array('modTindakan'=>$modTindakan),true));?>';
    //check tindakan 
...
function batalTindakan(obj){
 var daftartindakan_nama = $(obj).parents("tr").find("span[name$='daftartindakan_nama']").text();
    myConfirm('Apakah anda akan menghapus tindakan konsultasi ' + daftartindakan_nama + ' ?',"Perhatian!",function(r) {
        if(r)
        {
...
{
    window.open('<?php echo $this->createUrl('printStatusJz'); ?>&pendaftaran_id='+pendaftaran_id,'printwin','left=100,top=100,width=480,height=640');
}
...
function autoPrint(){
    ...
    <?php if(Yii::app()->user->getState('printkunjunganlsng')==TRUE){ ?>
        window.open('<?php echo $this->createUrl('printStatusJz',array('pendaftaran_id'=>$model->pendaftaran_id)); ?>','printwin','left=100,top=100,width=480,height=640');
    <?php  } ?>
}
