...
class CaraMasukRawatInapController extends MyAuthController {
	...
	public function actionSetIFrameDashboard() {
		...
		switch ($model->jns_periode) {
			case 'bulan' : $sql = "
										SELECT 
										date_trunc('month', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj
										FROM ricaramasuk_r
			...
			case 'tahun' : $sql = "
										SELECT 
										date_trunc('year', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj
										FROM ricaramasuk_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
			...
			default : $sql = "
										SELECT 
										date_trunc('day', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj
										FROM ricaramasuk_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
		...
		$sql = "
				SELECT 
				sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj
				FROM ricaramasuk_r
				WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
				";
		...
		foreach ($dataPie as $key => $value) {
			if ($key == "jumlah_ri") {
				$key = "Langsung Rawat Inap";
			} elseif ($key == "jumlah_rd") {
				$key = "Melalui Rawat Darurat";
			} else {
				$key = "Melalui Rawat Jalan";
			}
			$temp['jenis'] = $key;
			$temp['jumlah'] = $value;
		...
		switch ($model->jns_periode) {
			case 'bulan' : $criteria->select = array('date_trunc(' . "'month'" . ', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
				break;
			case 'tahun' : $criteria->select = array('date_trunc(' . "'year'" . ', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
				break;
			default : $criteria->select = array('date_trunc(' . "'day'" . ', tanggal) as periode, sum(langsung) as jumlah_ri, sum(darird) as jumlah_rd, sum(darirj) jumlah_rj');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
		}
