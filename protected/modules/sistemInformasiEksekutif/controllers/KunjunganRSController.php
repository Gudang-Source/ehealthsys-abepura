...
class KunjunganRSController extends MyAuthController {
	...
	public function actionSetIFrameDashboard() {
		...
		switch ($model->jns_periode) {
			case 'bulan' : $sql = "
										SELECT 
										date_trunc('month', tanggal) as periode, sum(kunjungan_ri) as jumlah_ri, sum(kunjungan_rd) as jumlah_rd, sum(kunjungan_rj) jumlah_rj
										FROM kunjunganrs_r
			...
			case 'tahun' : $sql = "
										SELECT 
										date_trunc('year', tanggal) as periode, sum(kunjungan_ri) as jumlah_ri, sum(kunjungan_rd) as jumlah_rd, sum(kunjungan_rj) jumlah_rj
										FROM kunjunganrs_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
			...
			default : $sql = "
										SELECT 
										date_trunc('day', tanggal) as periode, sum(kunjungan_ri) as jumlah_ri, sum(kunjungan_rd) as jumlah_rd, sum(kunjungan_rj) jumlah_rj
										FROM kunjunganrs_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
		...
		$sql = "
				SELECT 
				sum(kunjungan_ri) as jumlah_ri, sum(kunjungan_rd) as jumlah_rd, sum(kunjungan_rj) jumlah_rj
				FROM kunjunganrs_r
				WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
				";
