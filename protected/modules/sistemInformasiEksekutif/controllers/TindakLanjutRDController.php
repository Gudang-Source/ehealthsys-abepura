...
class TindakLanjutRDController extends MyAuthController {
    ...
    public function actionSetIFrameDashboard() {
        ...
        switch ($model->jns_periode) {
            case 'bulan' : $sql = "
		SELECT 
		date_trunc('month', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal
		FROM tindaklanjutigd_r
		...
		";
            ...
            case 'tahun' : $sql = "
		SELECT 
		date_trunc('year', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal
		FROM tindaklanjutigd_r
		WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
		GROUP BY periode
		ORDER BY periode ASC
		";
            ...
            default : $sql = "
		SELECT 
		date_trunc('day', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal
		FROM tindaklanjutigd_r
		WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
		GROUP BY periode
		ORDER BY periode ASC
        ...
        $sql = "
            SELECT 
            sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal
            FROM tindaklanjutigd_r
            WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
				";
        ...
        foreach ($dataPie as $key => $value) {
            if ($key == "jumlah_dirawat") {
                $key = "Rawat Inap";
            } elseif ($key == "jumlah_dirujuk") {
                $key = "Rawat Jalan";
            } elseif ($key == "jumlah_pulang") {
                $key = "Pasien Pulang";
            } else {
                $key = "Pasien Meninggal";
            }
            $temp['jenis'] = $key;
            $temp['jumlah'] = $value;
        ...
        switch ($model->jns_periode) {
            case 'bulan' : $criteria->select = array('date_trunc(' . "'month'" . ', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal');
                $criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
                $criteria->group = 'periode';
                $criteria->order = 'periode ASC';
                break;
            case 'tahun' : $criteria->select = array('date_trunc(' . "'year'" . ', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal');
                $criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
                $criteria->group = 'periode';
                $criteria->order = 'periode ASC';
                break;
            default : $criteria->select = array('date_trunc(' . "'day'" . ', tanggal) as periode, sum(dirawat) as jumlah_dirawat, sum(dirujuk) as jumlah_dirujuk, sum(pulang) jumlah_pulang, sum(meninggal) as jumlah_meninggal');
                $criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
                $criteria->group = 'periode';
                $criteria->order = 'periode ASC';
        }
