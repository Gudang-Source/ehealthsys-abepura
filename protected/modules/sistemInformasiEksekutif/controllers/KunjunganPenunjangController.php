...
class KunjunganPenunjangController extends MyAuthController {
	...
	public function actionSetIFrameDashboard() {
		...
		switch ($model->jns_periode) {
			case 'bulan' : $sql = "
										SELECT 
										date_trunc('month', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio
			...
			case 'tahun' : $sql = "
										SELECT 
										date_trunc('year', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio
										FROM kunjunganpenunjang_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
			...
			default : $sql = "
										SELECT 
										date_trunc('day', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio
										FROM kunjunganpenunjang_r
										WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
		...
		$sql = "
				SELECT 
				sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio
				FROM kunjunganpenunjang_r
				WHERE DATE(tanggal) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
				";
		...
		switch ($model->jns_periode) {
			case 'bulan' : $criteria->select = array('date_trunc(' . "'month'" . ', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
				break;
			case 'tahun' : $criteria->select = array('date_trunc(' . "'year'" . ', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
				break;
			default : $criteria->select = array('date_trunc(' . "'day'" . ', tanggal) as periode, sum(laboratorium) as jumlah_lab, sum(radiologi) as jumlah_radio');
				$criteria->addBetweenCondition('DATE(tanggal)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
		}
