...
class EmergencyResponseController extends MyAuthController {

	public $path_view = 'sistemInformasiEksekutif.views.emergencyResponse.';
	...
	public function actionSetIFrameDashboard() {
		...
		switch ($model->jns_periode) {
			case 'bulan' : $sql = "
										SELECT 
										date_trunc('month', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr
			...
			case 'tahun' : $sql = "
										SELECT 
										date_trunc('year', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr
										FROM rl1_2_indikatorpelayananrumahsakit
										WHERE DATE(tgl_laporan) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
			...
			default : $sql = "
										SELECT 
										date_trunc('day', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr
										FROM rl1_2_indikatorpelayananrumahsakit
										WHERE DATE(tgl_laporan) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
										GROUP BY periode
										ORDER BY periode ASC
		...
		$sql = "
				SELECT 
				sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr
				FROM rl1_2_indikatorpelayananrumahsakit
				WHERE DATE(tgl_laporan) BETWEEN '" . $model->tgl_awal . "' AND '" . $model->tgl_akhir . "'
				";
		...
		switch ($model->jns_periode) {
			case 'bulan' : $criteria->select = array('date_trunc(' . "'month'" . ', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr');
				...
				break;
			case 'tahun' : $criteria->select = array('date_trunc(' . "'year'" . ', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr');
				$criteria->addBetweenCondition('DATE(tgl_laporan)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
				break;
			default : $criteria->select = array('date_trunc(' . "'day'" . ', tgl_laporan) as periode, sum(ndr) as jumlah_ndr, sum(gdr) as jumlah_gdr');
				$criteria->addBetweenCondition('DATE(tgl_laporan)', $model->tgl_awal, $model->tgl_akhir);
				$criteria->group = 'periode';
				$criteria->order = 'periode ASC';
		}
