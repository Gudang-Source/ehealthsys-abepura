<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table border="0" style="padding :none;">
  <tr>
    <td>1. </td>
    <td>Icon <i class="icon-calendar"></i><i class="icon-time"></i>untuk menentukan tanggal transaksi.</td>
  </tr>
  <tr>
    <td>2. </td>
    <td>Icon <i class="icon-list-alt"></i><i class="icon-search"></i> berfungsi untuk menampilkan list data secara detail.</td>
  </tr>
  <tr>
    <td>3. </td>
    <td>Icon <i class="icon-remove"></i> berfungsi untuk membatalkan barang.</td>
  </tr>
  <tr>
    <td>4. </td>
    <td>Gunakan tombol ini  <button class="btn btn-primary" name="yt0" type="submit"><i class="icon-search icon-white"></i>
Simpan
</button> berfungsi untuk menyimpan</td>
  </tr>
  <tr>
    <td>5. </td>
    <td>Gunakan tombol ini <a class="btn btn-danger" href="/simrs/index.php?r=rawatInap/pasienRawatInap/formTindakLanjutDariPasienRI">
<i class="icon-refresh icon-white"></i>
Ulang
</a> untuk mengulang kembali inputan.</td>
  </tr>
  <tr>
    <td>6. </td>
    <td>Gunakan tombol ini <a class="btn btn-danger" href="/simrs/index.php?r=rawatInap/pasienRawatInap/formTindakLanjutDariPasienRI">
<i class="icon-refresh icon-white"></i>
Ulang
</a> untuk membatalkan aksi ini</td>
  </tr>
  <tr>
    <td>7. </td>
    <td>Gunakan tombol ini <a class="btn btn-info" href="/simrs/index.php?r=rawatInap/pasienRawatInap/formTindakLanjutDariPasienRI">
<i class="icon-info-sign icon-white"></i>
Petunjuk
</a> untunk melihat petunjuk.</td>
  </tr>
</table>
</p>
