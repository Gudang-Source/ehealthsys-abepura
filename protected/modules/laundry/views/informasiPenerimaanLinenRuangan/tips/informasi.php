<ol class="row-fluid">
    <div class="span6">
        <li>
            Tombol  <div class="btn btn-primary" name="yt0" type="button"><i class="icon-search icon-white"></i> Cari</div>
            berfungsi untuk mencari data.
        </li>
        <li>
            Icon  <i class="icon-form-silang"></i>
            berfungsi untuk membatalkan penerimaan.
        </li>
        <li>
            Tombol  <div class="btn btn-success" name="yt0" type="button"><i class="icon-ok icon-white"></i> Proses</div>
            berfungsi untuk melanjutkan ke transaksi penerimaan linen.
        </li>
        <li>
            Tombol  <div class="btn btn-danger" name="yt0" type="button"><i class="icon-refresh icon-white"></i> Ulang</div>
            berfungsi untuk memuat ulang halaman.
        </li>
        <li>
            Icon <i class="icon-form-lihat"></i></i> untuk melihat rincian data.
        </li>
    </div>
</ol>
