...
class PermintaanPenawaranController extends MyAuthController
{
    ...
    public $path_view = 'gudangFarmasi.views.permintaanPenawaran.';
    ...
    public function actionIndex($permintaanpenawaran_id=null,$rencana_id = null)
    {
        ...
        $modPenawaranDetail = new GFPenawaranDetailT;
        $modObatAlkes = new GFObatalkesM;
        ...
        if(!empty($rencana_id) && (empty($permintaanpenawaran_id))){
            $modRencanaKebFarmasi = GFRencanaKebFarmasiT::model()->findByPk($rencana_id);
            if (count($modRencanaKebFarmasi) == 1){
                $modDetailRencana = GFRencDetailkebT::model()->findAllByAttributes(array('rencanakebfarmasi_id'=>$rencana_id));
                ...
                if (count($modDetailRencana) > 0){
                    foreach ($modDetailRencana as $i => $rencana) {
                        ...
                        $modDetails[$i] = new GFPenawaranDetailT();
        ...
        if(!empty($permintaanpenawaran_id)){
            $modPermintaanPenawaran= GFPermintaanPenawaranT::model()->findByPk($permintaanpenawaran_id);
            ...
            $modDetails = GFPenawaranDetailT::model()->findAllByAttributes(array('permintaanpenawaran_id'=>$modPermintaanPenawaran->permintaanpenawaran_id));
        }
         ...
         if(isset($_POST['GFPermintaanPenawaranT'])){
             ...
             try {
					...
					if(isset($_GET['ubah'])){
						...
						$modPermintaanPenawaran->pegawaimengetahui_id = $_POST['GFPermintaanPenawaranT']['pegawaimengetahui_id'];
						$modPermintaanPenawaran->pegawaimenyetujui_id = $_POST['GFPermintaanPenawaranT']['pegawaimenyetujui_id'];
						$modPermintaanPenawaran->tglpenawaran = $format->formatDateTimeForDb($_POST['GFPermintaanPenawaranT']['tglpenawaran']);
						$modPermintaanPenawaran->ispenawaranmasuk = $_POST['GFPermintaanPenawaranT']['ispenawaranmasuk'];
					}else{
                    ...
                    if($modPermintaanPenawaran->save()){
						if(isset($_GET['ubah'])){
							$modDetailPenawaran = GFPenawaranDetailT::model()->deleteAllByAttributes(array('permintaanpenawaran_id'=>$modPermintaanPenawaran->permintaanpenawaran_id));
						}
						if(count($_POST['GFPenawaranDetailT']) > 0){
							foreach($_POST['GFPenawaranDetailT'] AS $i => $postOa){
...
//        CDateTimeParser::parse($modPermintaanPenawaran->tglpenawaran, 'yyyy-MM-dd hh:mm:ss')); 
    ...
    public function simpanDetailPenawaran($modPermintaanPenawaran ,$post){
        $format = new MyFormatter();
        $modDetailPenawaran = new GFPenawaranDetailT;
    ...
    public function actionPrint($permintaanpenawaran_id,$caraPrint = null) 
    {
        $format = new MyFormatter;    
        $modPermintaanPenawaran = GFPermintaanPenawaranT::model()->findByPk($permintaanpenawaran_id);     
        $modPenawaranDetail = GFPenawaranDetailT::model()->findAllByAttributes(array('permintaanpenawaran_id'=>$permintaanpenawaran_id));
