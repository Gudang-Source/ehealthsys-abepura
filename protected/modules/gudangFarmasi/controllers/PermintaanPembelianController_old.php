...
class PermintaanPembelianController extends MyAuthController
{
    ...
    public function actionIndex($permintaanpembelian_id = null,$rencana_id = null, $penawaran_id = null){
        ...
        if (!empty($penawaran_id) && (empty($permintaanpembelian_id))){
            $modPermintaanPenawaran = GFPermintaanPenawaranT::model()->findByPk($penawaran_id);
            $jumlahPermintaan = GFPermintaanpembelianT::model()->countByAttributes(array('permintaanpenawaran_id'=>$modPermintaanPenawaran->permintaanpenawaran_id));
            ...
            if (count($modPermintaanPenawaran)==1){
                ...
                if (count($modPenawaranDetail) > 0){
                    ...
                    foreach ($modPenawaranDetail as $key => $value) {                                
                        ...
                        $modDetails[$key] = new GFPermintaanDetailT();
            ...
            if ($modRencanaKebFarmasi){
                ...
                if (count($modRencanaDetail) > 0){
                    ...
                    foreach ($modRencanaDetail as $i => $value) {
                        $obat = GFObatalkesM::model()->findByPk($value->obatalkes_id);
                        $stok = StokobatalkesT::getJumlahStok($obat->obatalkes_id, Yii::app()->user->getState('ruangan_id'));
                        $jmlKemasan = ($value->kemasanbesar > 0) ? $value->kemasanbesar : $obat->kemasanbesar;
                        $modDetails[$i] = new GFPermintaanDetailT();
        ...
        if(!empty($permintaanpembelian_id)){
            ...
            $modDetails = GFPermintaanDetailT::model()->findAllByAttributes(array('permintaanpembelian_id'=>$modPermintaanPembelian->permintaanpembelian_id));
        }
        ...
        if(isset($_POST['GFPermintaanpembelianT'])){
            ...
            try {
					...
					if(isset($_GET['ubah'])){
						...
						$modPermintaanPembelian->tglpermintaanpembelian=$format->formatDateTimeForDb($_POST['GFPermintaanpembelianT']['tglpermintaanpembelian']);
                    ...
                    if($modPermintaanPembelian->save()){
							if(isset($_GET['ubah'])){
								$modPermintaanPembelianDetail = GFPermintaanDetailT::model()->deleteAllByAttributes(array('permintaanpembelian_id'=>$modPermintaanPembelian->permintaanpembelian_id));
							}
                            if(count($_POST['GFPermintaanDetailT']) > 0){
                               foreach($_POST['GFPermintaanDetailT'] AS $i => $postOa){
     ...
     /**
     * simpan GFPermintaanDetailT
     * @param type $modPermintaanPembelian
     * @param type $post
     * @return \GFPermintaanDetailT
     */
    public function simpanPermintaanPembelian($modPermintaanPembelian ,$post){
        $format = new MyFormatter();
        $modPermintaanPembelianDetail = new GFPermintaanDetailT;
...
//            $modPermintaanPembelianDetail->stokakhir = StokobatalkesT::getJumlahStok($obatalkes_id, Yii::app()->user->getState('ruangan_id'));
    ...
    public function actionPrint($permintaanpembelian_id,$caraPrint = null) 
    {
        $format = new MyFormatter;    
        $modPermintaanPembelian = GFPermintaanpembelianT::model()->findByPk($permintaanpembelian_id);     
        $modPermintaanPembelianDetail = GFPermintaanDetailT::model()->findAllByAttributes(array('permintaanpembelian_id'=>$permintaanpembelian_id));
