<span class="required"><i>Bagian dengan tanda * harus diisi.</i></span>
<p>
<table border="0" style="padding :none;">
  <tr>
    <td>1. </td>
    <td>Icon <i class="icon-calendar"></i><i class="icon-time"></i> untuk menentukan tanggal dan waktu pendaftaran.</td>
  </tr>
    <tr>
      <td>2. </td>
        <td>Gunakan icon ini <i class="icon-list"></i> berfungsi untuk memasukan volume fisik secara masal.</td>
  </tr>.
    <tr>
      <td>3. </td>
        <td>Gunakan icon ini <span class="add-on">
<a id="" href="javascript:void(0);" onclick="$("#dialogObatAlkes").dialog("open");return false;">
<i class="icon-list-alt"></i>
<i class="icon-search"></i>
</a>
</span> berfungsi untuk mencari dan menampilkan list datanya.</td>
  </tr>
  
   <tr>  <td>4. </td>
        <td>Gunakan tombol ini <a id="" class="btn btn-primary" type="button" name="yt3" rel="tooltip">
<i class="icon-search icon-white"></i>
Cari</a> berfungsi untuk mencari data</td>
 </tr>
     <tr>  <td>5. </td>
        <td>Gunakan tombol ini  <a class="btn btn-primary" name="yt0" >
<i class="icon-ok icon-white"></i>
Simpan
</a> berfungsi untuk menyimpan</td>
 </tr> 
 
 <tr>
    <td>6. </td>
    <td>Gunakan tombol ini <a class="btn btn-danger">
<i class="icon-refresh icon-white"></i>
Ulang
</a> berfungsi untuk mengulang kembali inputan.</td>
  </tr>
</table>
</p>

