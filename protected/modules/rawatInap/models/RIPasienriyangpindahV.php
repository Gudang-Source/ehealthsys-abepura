...
class RIPasienriyangpindahV extends PasienriyangpindahV
{
    /**
    ...
    public function searchRI()
	{
		...
		if(!empty($instalasiasal_id)){
			$criteria->addCondition("instalasiasal_id = ".$instalasiasal_id); 	
		}
		...
		if($this->ceklis)
		{
			$criteria->addCondition('DATE(tglpindahkamar) BETWEEN \''.$this->tgl_awal.'\' AND \''.$this->tgl_akhir.'\'');
		}
        ...
        public function getTindakanDanObat(){
            ...
            $retVal['jmlobat'] = 0;
            $retVal['msg'] = "";
            $modTindakan = TindakanpelayananT::model()->findAllByAttributes(array('pasienadmisi_id'=>$this->pasienadmisi_id, 'ruangan_id'=> $this->ruangan_id));
