...
$arrMenu = array();
                (Yii::app()->user->checkAccess(Params::DEFAULT_ADMIN)) ?array_push($arrMenu,array('label'=>Yii::t('mds','Manage').' PSJeniskegiatanpersalinanM ', 'header'=>true, 'itemOptions'=>array('class'=>'heading-master'))) :  '' ;
...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('psjeniskegiatanpersalinan-m-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php $this->widget('ext.bootstrap.widgets.BootGridView',array(
	'id'=>'psjeniskegiatanpersalinan-m-grid',
...
{
    window.open("${urlPrint}/"+$('#psjeniskegiatanpersalinan-m-search').serialize()+"&caraPrint="+caraPrint,"",'location=_new, width=900px');
}
JSCRIPT;
