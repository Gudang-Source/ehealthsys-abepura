...
class PersalinanTController extends MyAuthController {
	...
	public function actionIndex2($id) {
		...
		if ($ishere) {
			...
			// var_dump($simpanGinekologi, $successRiwayatKehamilan, $simpanPersalinan, $simpanPemeriksaan); die;
			
			if ($simpanGinekologi && $successRiwayatKehamilan && $simpanPersalinan && $simpanPemeriksaan) {
				$trans->commit();
				Yii::app()->user->setFlash('success',"Data Berhasil disimpan ");
				$this->redirect(Yii::app()->createUrl($this->module->id.'/persalinanT/index&id='.$id.'&sukses=1'));
			}
