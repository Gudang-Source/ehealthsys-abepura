...
$data = $model->searchTableKelas();
$template = "{summary}\n{items}\n{pager}";
$sort = true;
if (isset($caraPrint)){
    $sort = false;
  $data = $model->searchPrintKelas();  
  $template = "{items}";
  if ($caraPrint == "EXCEL")
...
<div>
    <?php if(!isset($caraPrint)){ ?>
        <legend class="rim"> Table Kinerja Per Kelas</legend>
        <?php 
...
<?php 
    ...
    $criteria->group = "t.daftartindakan_nama,kelompoktindakan_nama,t.ruanganpenunj_id,no_rekam_medik,nama_pasien,tarif_satuan";
        ...
        if (isset($_GET['BSLaporankinerjapenunjangV']['kelaspelayanan_id'])){
            $kelaspelayanan_id = $_GET['BSLaporankinerjapenunjangV']['kelaspelayanan_id'];
        }
