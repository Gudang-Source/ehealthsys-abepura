<?php
class InformasiPengajuanSterilisasiRuanganController extends MyAuthController {
	public $path_view = 'sterilisasi.views.informasiPengajuanSterilisasiRuangan.';
	...
	public function actionBatalPengajuanSteril($id){
		if(Yii::app()->request->isAjaxRequest)
		{
			$data['sukses']=0;
			$deleteDetail = STPengajuansterlilisasidetT::model()->deleteAllByAttributes(array('pengajuansterlilisasi_id'=>$id));
			$deletePengajuan = STPengajuansterlilisasiT::model()->deleteByPk($id);			
			 if($deleteDetail && $deletePengajuan){
				$data['sukses'] = 1;
			 }
