<tr>
    <td>
		...
		<?php echo CHtml::activeCheckBox($detail,'[ii]checklist', array('class'=>'checklist','onclick'=>'setNol(this);')); ?>
    </td>
	<td>
	...
	<td>
		<?php   
				$detail->pemeliharaanasetdet_tgl = (!empty($detail->pemeliharaanasetdet_tgl) ? date("Y-m-d H:i:s",strtotime($detail->pemeliharaanasetdet_tgl)) : null);
	...
	<td>
        <?php 
			echo CHtml::activeDropDownList($detail, '[ii]kondisiaset', CHtml::listData(MALookupM::jenis(),'lookup_id','lookup_name'),array('empty'=>'Kodisi Aset', 'style'=>'width:80px;'));
		?>
    </td>
