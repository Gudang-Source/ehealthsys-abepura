...
<div class="panel panel-primary" id="charts_env">
	...
	<div class="panel-heading">
		<div class="panel-title">Inventarisasi Aset Bulan Ini</div>
		...
		<thead>
			<tr>
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Pemeliharaan Aset Bulan Ini</div>
				...
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Penyusutan Aset Bulan Ini</div>
...
$(document).ready(function($) 
{
	...
	Morris.Area({
		...
		labels: ['Inventarisasi Tanah','Inventarisasi Peralatan','Inventarisasi Tanah','Inventarisasi Gedung','Inventarisasi Jalan'],
		parseTime: false,
		...
		data: [
			...
			if(count($dataLineChart) > 0){
				foreach ($dataLineChart as $i => $chart) { ?>
					{ x: "<?php echo date("d",strtotime($chart['tgl_pendaftaran'])); ?>", 
						y_1: <?php echo isset($chart['jumlah_1']) ? $chart['jumlah_1'] : 0; ?>, 
						y_2: <?php echo isset($chart['jumlah_2']) ? $chart['jumlah_2'] : 0; ?>, 
						y_3: <?php echo isset($chart['jumlah_3']) ? $chart['jumlah_3'] : 0; ?>,
						y_4: <?php echo isset($chart['jumlah_4']) ? $chart['jumlah_4'] : 0; ?> },
    ...
    var donut_chart = Morris.Donut({
		...
		if(count($dataDonutChart) > 0){
			foreach ($dataDonutChart as $i => $chart) { ?>
				{label: "<?php echo $chart['label']; ?>", value: <?php echo $chart['jumlah']; ?>},
			<?php }
		}?>
        ],
        colors: [
