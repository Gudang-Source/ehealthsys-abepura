...
.groupUkurans{
    ...
    <?php
        $this->renderPartial($this->path_view.'cutiMelahirkan/suratCutiMelahirkan',array('model'=>$model,'modPasien'=>$modPasien,
...
<?php
if(!empty($_GET['suratketerangan_id'])){
    $urlPrint=  Yii::app()->createAbsoluteUrl($this->module->id.'/'.$this->id.'/PrintCutiMelahirkan&pendaftaran_id='.$_GET['pendaftaran_id'].'&suratketerangan_id='.$_GET['suratketerangan_id'].'&lama_hari='.$_GET['lama_hari']);
...
<script>
    function cekValidasi(){
        ...
        var pols = $('#pols').val();
        var rr = $('#rr').val();
        var usia_kehamilan = $('#SuratketeranganR_usiakehamilan').val();
        
        if(tekanan_darah == '' || tempratur == '' || pols == '' || rr == '' || usia_kehamilan == ''){
