...
<div class="panel panel-primary" id="charts_env">
	...
	<div class="panel-heading">
		<div class="panel-title">Transaksi Dokumen Rekam Medis Bulan Ini</div>
...
$(document).ready(function($) 
{
        ...
        data: [
			...
			if(count($dataAreaChart) > 0){
				foreach ($dataAreaChart as $i => $chart) { ?>
					{ "x": "<?php echo date("d",strtotime($chart['tglrekammedis'])); ?>", "y": <?php echo $chart['jumlah']; ?> },
				<?php } 
			}
			?>
		],
 ...
 // Line Charts
    Morris.Line({
        ...
        data: [
            ...
            if(count($dataLineChart) > 0){
                foreach ($dataLineChart as $i => $chart) { ?>
                    { x: "<?php echo date("d",strtotime($chart['tglpeminjamanrm'])); ?>", y_1: <?php echo isset($chart['jumlah_1']) ? $chart['jumlah_1'] : 0; ?>, y_2: <?php echo isset($chart['jumlah_2']) ? $chart['jumlah_2'] : 0; ?> },
                <?php }
            }
            ?>
        ...
        xkey: 'x',
        ykeys: ['y_1','y_2'],
        labels: ['Jumlah Pengiriman Dokumen', 'Jumlah Peminjaman Dokumen'],
        parseTime: false,
