...
<?php $form=$this->beginWidget('ext.bootstrap.widgets.BootActiveForm',array(
	...
	'enableAjaxValidation'=>false,
        ...
        'focus' => '#RKPemeriksaanFisikT_keadaanumum_annoninput .maininput',
)); ?>
<style>
.groupUkurans{
        display:inline;
    ...
    <div class="span4">
        <fieldset class='box'>
			...
			<div class="control-group ">
				...
				<div class="controls">
					<?php echo $form->dropDownList($modPemeriksaanFisik,'paramedis_nama', CHtml::listData($modPemeriksaanFisik->getParamedisItems($modPendaftaran->ruangan_id), 'pegawai.nama_pegawai', 'pegawai.nama_pegawai'),array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event);", 'maxlength' => 100)); ?>
				</div>
			</div>
				<!--<div class="control-group ">
                                            ...
                                            <?php $crit = new CDbCriteria();
                                                    ...
                                                    $crit->order = 'metodegcs_nilai ASC';
                                                     echo $form->dropDownList($modPemeriksaanFisik,'gcs_eye',  
                                                                    CHtml::listData(RKMetodeGCSM::model()->findAll($crit), 'metodegcs_nilai', 'textMetodeGCSM'),array('empty'=>'-- Pilih --', 'class'=>'span3', 'onkeypress'=>"return $(this).focusNextInputField(event);",'onchange'=>'hitungCGS()')); ?>
                                    </div>
                            </div>
                            <div class="control-group ">
                                    ...
                                    <div class="controls">
                                            ...
                                            echo $form->dropDownList($modPemeriksaanFisik,'gcs_verbal',
                                                            CHtml::listData(RKMetodeGCSM::model()->findAll($crit3), 'metodegcs_nilai', 'textMetodeGCSM'),array('empty'=>'-- Pilih --', 'class'=>'span3', 'onkeypress'=>"return $(this).focusNextInputField(event);",'onchange'=>'hitungCGS()')); ?>
                                    </div>
                            </div>
                            <div class="control-group ">
                                    ...
                                    <div class="controls">
                                            ...
                                            echo $form->dropDownList($modPemeriksaanFisik,'gcs_motorik',
                                                            CHtml::listData(RKMetodeGCSM::model()->findAll($crit2), 'metodegcs_nilai', 'textMetodeGCSM'),array('empty'=>'-- Pilih --', 'class'=>'span3', 'onkeypress'=>"return $(this).focusNextInputField(event);",'onchange'=>'hitungCGS()')); ?>
                                    </div>
                            </div>
                            <div class="control-group ">
...
$getTextTekananDarah = Yii::app()->createUrl('rekamMedis/periksaFisikTRK/GetTextTekananDarah');
...
$getBMIText = Yii::app()->createUrl('rekamMedis/periksaFisikTRK/getBMIText');
...
function SetNilai(obj)
{
    ...
    i=0;
    if(idTombol=='E'){
        $('#RKPemeriksaanFisikT_gcs_eye').val(valueGCS);
    }else if(idTombol=='M'){
        $('#RKPemeriksaanFisikT_gcs_motorik').val(valueGCS);
    }else if(idTombol=='V'){
        $('#RKPemeriksaanFisikT_gcs_verbal').val(valueGCS);
    } 
...
function hitungCGS()
{
    gcs_eye =  $('#RKPemeriksaanFisikT_gcs_eye').val();
    gcs_motorik =  $('#RKPemeriksaanFisikT_gcs_motorik').val();
    gcs_verbal =  $('#RKPemeriksaanFisikT_gcs_verbal').val();    
    if((gcs_eye!='') && (gcs_motorik!='') &&(gcs_verbal!='')){
        $.post("${urlgetMetodeGCS}",{gcs_eye: gcs_eye,gcs_motorik:gcs_motorik,gcs_verbal:gcs_verbal},
        function(data){
               if(data.pesan==null){
                 $('#RKPemeriksaanFisikT_namaGCS').val(data);
               }else{
                    myAlert(data.pesan);
               }    
        },"json");
