...
class InformasiantrianController extends MyAuthController
{
	...
	public $path_view = 'pendaftaranPenjadwalan.views.informasiantrian.';
	...
	public function actionCreate()
	{
		...
		$model=new RKInformasiantrianpasienV;

		// Uncomment the following line if AJAX validation is needed
		

		if(isset($_POST['RKInformasiantrianpasienV']))
		{
			$model->attributes=$_POST['RKInformasiantrianpasienV'];
			if($model->save()){
	...
	{
		$dataProvider=new CActiveDataProvider('RKInformasiantrianpasienV');
...
//                if(!Yii::app()->user->checkAccess(Params::)){throw new CHttpException(401,Yii::t('mds','You are prohibited to access this page. Contact Super Administrator'));}
		$model=new RKInformasiantrianpasienV('search');
		$format = new MyFormatter();
//		$model->unsetAttributes();  // clear any default values
		$model->tgl_awal = date('Y-m-d');
		$model->tgl_akhir = date('Y-m-d');
		if(isset($_GET['RKInformasiantrianpasienV'])){
			$model->attributes=$_GET['RKInformasiantrianpasienV'];
	...
	{
		$model=RKInformasiantrianpasienV::model()->findByPk($id);
		if($model===null)
        ...
        public function actionRemoveTemporary($id)
	{
        ...
        {
            $model= new RKInformasiantrianpasienV;
            $model->attributes=$_REQUEST['RKInformasiantrianpasienV'];
            $judulLaporan='Data RKInformasiantrianpasienV';
