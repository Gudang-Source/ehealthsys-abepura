...
class KursrpMController extends MyAuthController
{
	...
	{
		$dataProvider=new CActiveDataProvider('AKKursrpM');
	...
	{
		$model=AKKursrpM::model()->findByPk($id);
		if($model===null)
...
//                    $this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('admin'));
        ...
        if(isset($_POST['id']))
        {
           $update = AKKursrpM::model()->updateByPk($id,array('kursrp_aktif'=>false));
           if($update)
            {
                if (Yii::app()->request->isAjaxRequest)
                {
        ...
        {
            $model= new AKKursrpM;
			if(isset($_REQUEST['AKKursrpM'])){
				$model->attributes=$_REQUEST['AKKursrpM'];
			}			
