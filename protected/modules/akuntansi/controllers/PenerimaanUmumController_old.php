		...
		if(Yii::app()->request->isAjaxRequest)
		{
			...
			if(isset($data_parsing['AKPenerimaanUmumT'])){
				...
				try {
						...
						$data_parsing['AKPenerimaanUmumT']['tglpenerimaan'] = $format->formatDateTimeForDb($data_parsing['AKPenerimaanUmumT']['tglpenerimaan']);

						$modPenUmum = $this->savePenerimaan($data_parsing['AKPenerimaanUmumT'], $modTandaBukti);
						if(isset($data_parsing['pakeAsuransi']))
						{
							if(isset($data_parsing['AKUraianpenumumT'])){
...
//			$criteria->params = $params;
    ...
    public function actionPrint($id) {
        $model = AKPenerimaanUmumT::model()->findByPk($id);
        $modUraian = AKUraianpenumumT::model()->findAllByAttributes(array('penerimaanumum_id'=>$model->penerimaanumum_id));
        
        $judulLaporan = '--- Detail Penerimaan Kas ---';
        $caraPrint = $_REQUEST['caraPrint'];
        if ($caraPrint == 'PRINT') {
            $this->layout = '//layouts/printWindows';
            $this->render('Print', array('model' => $model, 'modUraian'=>$modUraian, 'judulLaporan' => $judulLaporan, 'caraPrint' => $caraPrint));
            ...
            $mpdf->WriteHTML($this->renderPartial('Print', array('model' => $model, 'modUraian'=>$modUraian,'judulLaporan' => $judulLaporan, 'caraPrint' => $caraPrint), true));
            $mpdf->Output();
         }
    }
...
//			$model = RekeningakuntansiV::model()->findAll($criteria);
			...
			$model = AKJenispenerimaanrekeningV::model()->findAll($criteria);
			if($model)
			{
				echo CJSON::encode(
