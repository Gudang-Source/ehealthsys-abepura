...
<div id="div_rekap">                
    <?php
        ...
        $criteria2->select ='t.*';
        if(isset($_GET['AKFakturpembelianT']['tglAwal'])){
            $tglAwal = $format2->formatDateTimeForDb($_GET['AKFakturpembelianT']['tglAwal']);
        }
        if(isset($_GET['AKFakturpembelianT']['tglAkhir'])){
            $tglAkhir = $format2->formatDateTimeForDb($_GET['AKFakturpembelianT']['tglAkhir']);
        }
//        echo $tglAwal; echo $tglAkhir;exit;
        $criteria2->addBetweenCondition('t.tglfaktur',$tglAwal,$tglAkhir);
        $model = AKFakturpembelianT::model()->findAll($criteria2);
        $supplier_temp = "";
        if(count($model) > 0){
    ...
    //            exit;
                ...
                $condition  = "supplier_m.supplier_nama ILIKE '%".$term."%' OR supplier_m.supplier_nama ILIKE '%".$term."%'";
                    ...
                    $criteria->group = 't.nofaktur,t.tglfaktur,t.tgljatuhtempo,t.keteranganfaktur,t.create_ruangan,t.fakturpembelian_id,
                                        ...
                                        penerimaanbarang_t.noterima,penerimaanbarang_t.tglterima,permintaanpembelian_t.nopermintaan';
                    $criteria->compare('t.supplier_id',$_GET['AKFakturpembelianT']['supplier_id']);
                    $criteria->compare('LOWER(t.nofaktur)',strtolower($_GET['AKFakturpembelianT']['nofaktur']),true);
                    $criteria->addBetweenCondition('t.tglfaktur',$tglAwal,$tglAkhir);
                ...
                $totBayar = 0;
                $totSisa = 0;
                $detail = AKFakturpembelianT::model()->findAll($criteria);
                foreach($detail as $key=>$details){
                        ...
                        echo "<tr>
                                  ...
                                  <td width='280px;'>".$details->tglterima."</td>
                                  ...
                                  <td width='280px;'>".$details->tglfaktur."</td>
                                  <td width='280px;'>".$details->tgljatuhtempo."</td>
...
//        echo $tglAwal; echo $tglAkhir;exit;
    $criteria2->addBetweenCondition('t.tglfaktur',$tglAwal,$tglAkhir);
    $criteria2->compare('LOWER(t.nofaktur)',strtolower($_GET['AKFakturpembelianT']['nofaktur']));
    $criteria2->compare('t.supplier_id',$_GET['AKFakturpembelianT']['supplier_id']);
    $model = AKFakturpembelianT::model()->findAll($criteria2);
    $supplier_temp = "";
    if(count($model) > 0){
    foreach($model as $key=>$models){
         if($models->supplier_id){
                ...
                $nopermintaan = !empty($models->penerimaan->permintaanpembelian_id)?$models->penerimaan->permintaanpembelian->nopermintaan:' - ';
            ...
            $criteria = new CDbCriteria;
            if(isset($_GET['AKFakturpembelianT']['tglAwal'])){
                $tglAwal = $format->formatDateTimeForDb($_GET['AKFakturpembelianT']['tglAwal']);
            }
            if(isset($_GET['AKFakturpembelianT']['tglAkhir'])){
                $tglAkhir = $format->formatDateTimeForDb($_GET['AKFakturpembelianT']['tglAkhir']);
            }
            $term = $supplier;
            ...
            $criteria->compare('fakturpembelian_t.supplier_id',$_GET['AKFakturpembelianT']['supplier_id']);
            $criteria->compare('LOWER(fakturpembelian_t.nofaktur)',strtolower($_GET['AKFakturpembelianT']['nofaktur']),true);
            $criteria->addBetweenCondition('fakturpembelian_t.tglfaktur',$tglAwal,$tglAkhir);
