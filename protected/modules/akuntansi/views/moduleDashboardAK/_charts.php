...
<div class="panel panel-primary" id="charts_env">
	...
	<div class="panel-heading">
		<div class="panel-title">Pengeluaran Umum Bulan Ini</div>
		...
		<thead>
			<tr>
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Penerimaan Umum (Non Kas)</div>
				...
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Penerimaan Umum (Kas)</div>
        ...
        data: [
			...
			if(count($dataAreaChart) > 0){
				foreach ($dataAreaChart as $i => $chart) { ?>
					{ "x": "<?php echo date("d",strtotime($chart['tglpengeluaran'])); ?>", "y": <?php echo $chart['jumlah']; ?> },
				<?php } 
			}
			?>
		],
		...
		data: [
			...
			if(count($dataLineChart) > 0){
				foreach ($dataLineChart as $i => $chart) { ?>
					{ x: "<?php echo date("d",strtotime($chart['tglpengeluaran'])); ?>", y_1: <?php echo isset($chart['jumlah_1']) ? $chart['jumlah_1'] : 0; ?>, y_2: <?php echo isset($chart['jumlah_2']) ? $chart['jumlah_2'] : 0; ?>},
				<?php }
			}
			?>
    ...
    var donut_chart = Morris.Donut({
		...
		if(count($dataDonutChart) > 0){
			foreach ($dataDonutChart as $i => $chart) { ?>
				{label: "<?php echo $chart['jenispengeluaran_nama']; ?>", value: <?php echo $chart['jumlah']; ?>},
			<?php }
		}?>
        ],
        colors: [
