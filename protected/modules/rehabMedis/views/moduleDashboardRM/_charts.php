...
<div class="panel panel-primary" id="charts_env">
	...
	<div class="panel-heading">
		<div class="panel-title">Pasien Rehabilitasi Medis Bulan Ini</div>
		...
		<thead>
			<tr>
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Non-Rujukan</div>
				...
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Rujukan</div>
...
$(document).ready(function($) 
{
        ...
        data: [
			...
			if(count($dataAreaChart) > 0){
				foreach ($dataAreaChart as $i => $chart) { ?>
					{ "x": "<?php echo date("d",strtotime($chart['tglmasukpenunjang'])); ?>", "y": <?php echo $chart['jumlah']; ?> },
				<?php } 
			}
			?>
		],
	...
	Morris.Line({
		...
		data: [
			...
			if(count($dataLineChart) > 0){
				foreach ($dataLineChart as $i => $chart) { ?>
					{ x: "<?php echo date("d",strtotime($chart['tglmasukpenunjang'])); ?>", y_1: <?php echo isset($chart['jumlah_1']) ? $chart['jumlah_1'] : 0; ?>, y_2: <?php echo isset($chart['jumlah_2']) ? $chart['jumlah_2'] : 0; ?> ,y_3: <?php echo isset($chart['jumlah_3']) ? $chart['jumlah_3'] : 0; ?> ,y_4: <?php echo isset($chart['jumlah_4']) ? $chart['jumlah_4'] : 0; ?>,y_5: <?php echo isset($chart['jumlah_5']) ? $chart['jumlah_5'] : 0; ?>,y_6: <?php echo isset($chart['jumlah_6']) ? $chart['jumlah_6'] : 0; ?> },
				<?php }
			}
			?>
		...
		labels: ['Fisioterapi','Okupasi Terapi','Terapi Wicara','Ortosis-Prostetis','Psikologi','Sosial Medik'],
		parseTime: false,
    ...
    var donut_chart = Morris.Donut({
		...
		if(count($dataDonutChart) > 0){
			foreach ($dataDonutChart as $i => $chart) { ?>
				{label: "<?php echo $chart['tindakanrm_nama']; ?>", value: <?php echo $chart['jumlah']; ?>},
			<?php }
		}?>
        ],
        colors: [
