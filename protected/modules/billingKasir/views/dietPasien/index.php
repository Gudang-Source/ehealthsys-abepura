...
if(isset($_GET['sukses']))
    Yii::app()->user->setFlash('success',"Data DietPasien berhasil disimpan!"); 
?>
    ...
    <?php $this->Widget('ext.bootstrap.widgets.BootAccordion',array(
        ...
        'content'=>array(
            'content-riwayattindakan'=>array(
                ...
                'isi'=>$this->renderPartial($this->path_view_rj.'_tableRiwayatTindakan',array(
                        'format'=>$format,
...
<?php $this->renderPartial($this->path_view_rj.'_jsFunctions', array('form'=>$form,'modTindakan'=>$modTindakan)); ?>
<?php $this->endWidget(); ?>
...
$this->beginWidget('zii.widgets.jui.CJuiDialog',
    array(
        'id'=>'dialog_tindakan',
        'options'=>array(
            'title'=>'Daftar Menu Diet '.(InstalasiM::model()->findByPk($instalasi_id)->instalasi_nama),
            'autoOpen'=>false,
            'modal'=>true,
            'width'=>860,
            'height'=>380,
...
$modDaftarTindakan = new BKMenuDietM('searchDialogDiet');
...
if (isset($_GET['BKMenuDietM'])){
    $modDaftarTindakan->attributes = $_GET['BKMenuDietM'];
    $modDaftarTindakan->tipepaket_id = $_GET['BKMenuDietM']['tipepaket_id'];
    $modDaftarTindakan->kategoritindakan_nama = $_GET['BKMenuDietM']['kategoritindakan_nama'];
    $modDaftarTindakan->daftartindakan_nama = $_GET['BKMenuDietM']['daftartindakan_nama'];
    $modDaftarTindakan->daftartindakan_kode = $_GET['BKMenuDietM']['daftartindakan_kode'];
    $modDaftarTindakan->kelaspelayanan_id = $_GET['BKMenuDietM']['kelaspelayanan_id'];
}
$this->widget('ext.bootstrap.widgets.BootGridView',
    array(
        'id'=>'daftartindakan-grid',
        'dataProvider'=>$modDaftarTindakan->searchDialogDiet(),
