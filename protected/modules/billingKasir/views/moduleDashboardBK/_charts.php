...
<div class="panel panel-primary" id="charts_env">
	...
	<div class="panel-heading">
		<div class="panel-title">Jumlah Pendapatan Dan Pengeluaran Bulan Ini</div>
		...
		<thead>
			<tr>
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Jumlah Setoran Harian</div>
				...
				<th width="50%" class="col-padding-1">
					<div class="pull-left">
						<div class="h4 no-margin">Jumlah Setoran Ke Bank</div>
...
$(document).ready(function($) 
{
        ...
        data: [
			...
			if(count($dataAreaChart) > 0){
				foreach ($dataAreaChart as $i => $chart) { ?>
					{ "x": "<?php echo date("d",strtotime($chart['tglpembayaran'])); ?>", "y": <?php echo $chart['jumlah']; ?> },
				<?php } 
			}
			?>
		],
	...
	Morris.Line({
		...
		data: [
			...
			if(count($dataLineChart) > 0){
				foreach ($dataLineChart as $i => $chart) { ?>
					{ x: "<?php echo date("d",strtotime($chart['ruangan_nama'])); ?>", y_1: <?php echo isset($chart['jumlah_1']) ? $chart['jumlah_1'] : 0; ?>, y_2: <?php echo isset($chart['jumlah_2']) ? $chart['jumlah_2'] : 0; ?> , y_3: <?php echo isset($chart['jumlah_3']) ? $chart['jumlah_3'] : 0; ?> , y_4: <?php echo isset($chart['jumlah_4']) ? $chart['jumlah_4'] : 0; ?> , y_5: <?php echo isset($chart['jumlah_5']) ? $chart['jumlah_5'] : 0; ?> , y_6: <?php echo isset($chart['jumlah_6']) ? $chart['jumlah_6'] : 0; ?> },
				<?php }
			}
			?>
		...
		labels: ['Kasir Karcis Poliklinik', 'Kasir Laboratorium', 'Kasir Radiologi', 'Kasir Apotek', 'Kasir Tindakan Poliklinik', 'Kasir IGD'],
		parseTime: false,
    ...
    var donut_chart = Morris.Donut({
		...
		if(count($dataDonutChart) > 0){
			foreach ($dataDonutChart as $i => $chart) { ?>
				{label: "<?php echo $chart['instalasi_nama']; ?>", value: <?php echo $chart['jumlah']; ?>},
			<?php }
		}?>
        ],
        colors: [
