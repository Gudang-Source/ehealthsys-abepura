...
function pilihPemeriksaanIni(obj){
    ...
    if($(obj).is(':checked')){
        ...
        if(delete_row.find('input[name$="[tindakanpelayanan_id]"][value=""]').length > 0){ //RND-6827
			delete_row.detach();
		}else{
...
function setCheckedPemeriksaanDariPermintaan(){
    ...
    $("#form-permintaankepenunjang").find('input[name$="[pemeriksaanrad_id]"]').each(function(){
        ...
        if(checkbox_pemeriksaan.val()){
			...
			var rowpermintaan = $("#form-tindakanpemeriksaan").find('input[name$="[pemeriksaanrad_id]"][value='+pemeriksaanrad_id+']');
