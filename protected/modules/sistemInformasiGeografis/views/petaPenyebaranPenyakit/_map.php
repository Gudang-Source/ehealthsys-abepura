<div class="row-fluid tile-block">
	<div class="col-md-3" style="padding:0">
		<div class="tile-header">
			<?php if($ada_diagnosa){ ?>
					<span style="font-size: 10pt">Penyebaran Diagnosa Terpilih<br>
					<?php echo ($ada_tiperumah)?'Berdasarkan Jenis Tempat Tinggal Pekerja,<br>Tipe Rumah :  '.$tiperumah_param.'<br>':''; ?>
					Dari Tanggal <?php echo MyFormatter::formatDateTimeForUser($tgl_awal); ?> s/d <?php echo MyFormatter::formatDateTimeForUser($tgl_akhir); ?> </span> 
			<?php }else{ ?>
					<span style="font-size: 10pt">Penyebaran 10 Besar Diagnosa <br>
					<?php echo ($ada_tiperumah)?'Berdasarkan Jenis Tempat Tinggal Pekerja,<br>Tipe Rumah :  '.$tiperumah_param.'<br>':''; ?>
					Dari Tanggal <?php echo MyFormatter::formatDateTimeForUser($tgl_awal); ?> s/d <?php echo MyFormatter::formatDateTimeForUser($tgl_akhir); ?> </span> 
			<?php } ?>
		</div>
		<div class="scrollable" data-height="400" data-scroll-position="right" data-rail-color="#fff" data-rail-opacity=".9" data-rail-width="8" data-rail-radius="10" data-autohide="1">
			<?php if(count($dataMap)>0){ foreach ($dataMap as $i => $map){ ?>
				<div class="tile-entry list-map" style="cursor:pointer;background:rgba(0, 0, 0, 0.15); padding:10px 10px 10px 10px; margin:1px 0" onclick="setPasien(this,<?php echo $map['diagnosa_id'] ?>);">
				   <span style="display:inline-block;color:#fff;font-size:11px;"><?php echo $map['diagnosa_kode'] ?> - <?php echo $map['diagnosa_nama'] ?></span>
...
function setPasien(obj,diagnosa_id){
    ...
    $.ajax({
        ...
        data: {diagnosa_id:diagnosa_id,ada_tiperumah:'<?php echo $ada_tiperumah; ?>',tgl_awal:'<?php echo $tgl_awal; ?>',tgl_akhir:'<?php echo $tgl_akhir; ?>'},
        dataType: "json",
...
function printasd(){
	...
	var diagnosa_id = $('#berdasarkan_diagnosa_id').val();
    window.open("<?php echo Yii::app()->createAbsoluteUrl(Yii::app()->controller->module->id.'/'.Yii::app()->controller->id.'/print'); ?>&tgl_awal="+tgl_awal+"&tgl_akhir="+tgl_akhir+"&diagnosa_id="+diagnosa_id,'printwin','left=100,top=100,width=1200px,scrollbars=yes');
}
