<div class="row-fluid tile-block">
	<div class="col-md-3" style="padding:0">
		<div class="tile-header">
			<h3 style="color:#fff">Legend</h3> <span>Penyebaran Pasien Rumah Sakit<br>Berdasarkan Kecamatan <br>Dari Tanggal <?php echo MyFormatter::formatDateTimeForUser($tgl_awal); ?> s/d <?php echo MyFormatter::formatDateTimeForUser($tgl_akhir); ?> </span> 
		</div>
		<div class="scrollable" data-height="400" data-scroll-position="right" data-rail-color="#fff" data-rail-opacity=".9" data-rail-width="8" data-rail-radius="10" data-autohide="1">
			<?php if(count($dataMap)>0){ foreach ($dataMap as $i => $map){ ?>
				<div class="tile-entry list-map" style="cursor:pointer;background:rgba(0, 0, 0, 0.15); padding:10px 10px 10px 10px; margin:1px 0" onclick="setPasien(this,<?php echo $map['kecamatan_id'] ?>);">
...
function print()
{
	var kecamatan_id = $("#berdasarkan_kecamatan_id").val();
    window.open("${urlPrint}&kecamatan_id="+kecamatan_id,"",'left=100,top=100,width=1200px,scrollbars=yes');
}
JSCRIPT;
Yii::app()->clientScript->registerScript('print',$jsx,CClientScript::POS_HEAD);                        
?> 

<?php echo CHtml::hiddenField('berdasarkan_kecamatan_id', '', array('readonly'=>true)); ?>
