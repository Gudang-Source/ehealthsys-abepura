        ...
        <table>
            <tr>
                <td width="50%">
                    ...
                    <?php // RND-30003>>> ?>
                        <?php //echo $form->dropDownListRow($modelPulang,'carakeluar', LookupM::getItems('carakeluar'),array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event)",'onchange'=>'carakeluar(this.value)')); ?>
<!--                     <div class="control-group ">
                        <?php //echo $form->labelEx($modelPulang,'tglpasienpulang', array('class'=>'control-label')) ?>
                        <?php //echo CHtml::label('Kondisi Keluar', 'kondisipulang', array('class'=>'control-label'))?>
                        <div class="controls">
                        <?php 
                            //echo $form->dropDownList($modelPulang,'kondisipulang', LookupM::getItems('kondisipulang'),array('empty'=>'-- Pilih --','onkeypress'=>"return $(this).focusNextInputField(event)",'onchange'=>'pasienmeninggal(this.value)')); 
                        ?>
                        </div>
                     </div>-->
...
<script>
    ...
    function pasienmeninggal(value)
    {
        if(value == "<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_1 ?>" || value == "<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_2 ?>")
        {
             ...
             $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').removeAttr('disabled');
        }
        else
        {
            $('#dialogPasienPulang #isDead').removeAttr('checked');
            $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').attr('disabled','true');
        }
    }
    $('#dialogPasienPulang #isDead').change(function(){
        if ($(this).is(':checked')){
                $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').removeAttr('disabled');
                $('#dialogPasienPulang #PasienpulangT_kondisipulang_id').val('<?php echo Params::KONDISIKELUAR_ID_MENINGGAL_1 ?>');
                $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').val('<?php echo date('Y-m-d H:i:s') ?>');
        }else{
                $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').attr('disabled','true');
                $('#dialogPasienPulang #PasienpulangT_kondisipulang_id').val('');
                $('#dialogPasienPulang #PasienpulangT_tgl_meninggal').val('');
    ...
    function konfirmasi()
    {
        ...
        {   
            $('#PasienpulangT_carakeluar_id').focus();
            return false;
        }
    }
</script>
